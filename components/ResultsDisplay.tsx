...
const MetricBox: React.FC<{ 
  ...
  sub?: string;
  description?: string;
}> = ({ title, value, suffix = "", colorClass = "text-white", sub, description }) => {
  const [showInfo, setShowInfo] = useState(false);

  return (
    <div className="p-4 bg-slate-950/50 rounded-xl border border-slate-800/60 hover:border-slate-700 transition-colors relative">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] text-slate-500 uppercase tracking-widest block">{title}</span>
        {description && (
          <button 
            onClick={() => setShowInfo(!showInfo)}
            className="text-slate-600 hover:text-indigo-400 transition-colors p-1 -mr-1"
      ...
      {showInfo && description && (
        <div className="absolute inset-0 z-20 bg-slate-900/95 p-3 rounded-xl border border-indigo-500/40 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-150">
          <p className="text-[9px] text-slate-300 leading-relaxed text-center italic px-1">
            {description}
          </p>
          <button 
    ...
    <div className="space-y-8 animate-fade-in">
      {!calculation && (
        <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-slate-800 rounded-xl bg-slate-950/30">
          ...
          <p className="text-slate-500 font-medium">Select an expiration date to view analysis.</p>
        </div>
      )}
      ...
      {calculation && (
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className={`rounded-2xl border p-6 flex flex-col justify-between ${calculation.isTargetMet ? 'bg-emerald-950/10 border-emerald-900/40' : 'bg-rose-950/10 border-rose-900/40'}`}>
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold text-slate-100 tracking-tight">Validation Check</h3>
                  <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter ${calculation.isTargetMet ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white shadow-lg shadow-rose-500/20'}`}>
                    {calculation.isTargetMet ? 'Target Met' : 'Yield Under Target'}
                  </div>
                </div>
              ...
              <div>
                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest mb-2">
                  ...
                  <span className={calculation.isTargetMet ? 'text-emerald-400' : 'text-rose-400'}>{((calculation.actualTotalCredit / calculation.requiredTotalCredit) * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ease-out ${calculation.isTargetMet ? 'bg-emerald-500' : 'bg-rose-500'}`} 
                    style={{ width: `${Math.min(100, (calculation.actualTotalCredit / calculation.requiredTotalCredit) * 100)}%` }}
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900/50 rounded-2xl border border-slate-800 p-6">
             <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-3">
                ...
                Raw Contract Snapshot: {calculation.option.ticker}
                <span className="flex-1 h-px bg-slate-800"></span>
             </h3>
             <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <MetricBox title="Strike" value={calculation.option.strike} suffix="" colorClass="text-slate-100" />
                <MetricBox 
                  title="Bid / Ask" 
                  value={calculation.option.bid !== null && calculation.option.ask !== null ? `${calculation.option.bid.toFixed(3)} / ${calculation.option.ask.toFixed(3)}` : null} 
                  colorClass="text-indigo-300" 
                  description="Bid is the highest price a buyer will pay; Ask is the lowest price a seller will accept."
                />
                <MetricBox title="Last Price" value={calculation.option.last?.toFixed(3) || null} colorClass="text-slate-400" />
                <MetricBox title="Volume" value={calculation.option.vol} colorClass="text-blue-400" />
                <MetricBox 
                  title="Open Int." 
                  value={calculation.option.oi} 
                  colorClass="text-purple-400" 
                  description="The total number of outstanding derivative contracts that have not been settled."
                />
                <MetricBox 
                  title="Delta" 
                  value={calculation.option.delta?.toFixed(3) || null} 
                  colorClass="text-amber-400" 
                  description="Measures the rate of change of the option value relative to the underlying asset's price."
                />
                <MetricBox 
                  title="Theta" 
                  value={calculation.option.theta?.toFixed(3) || null} 
                  colorClass="text-orange-400" 
                  description="Measures the rate of decline in the option's value due to the passage of time (time decay)."
                />
